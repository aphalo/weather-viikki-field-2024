...
clear_sky_PAR_total_rel.fun <- 
  function(x) {clear_sky_PAR_total.fun(x) / clear_sky_PAR_total.fun(90)}
clear_sky_UVB_total_rel.fun <- 
  function(x) {clear_sky_UVB_total.fun(x) / clear_sky_UVB_total.fun(90)}
clear_sky_UVA2_total_rel.fun <- 
  function(x) {clear_sky_UVA2_total.fun(x) / clear_sky_UVA2_total.fun(90)}
clear_sky_UVA1_total_rel.fun <- 
  function(x) {clear_sky_UVA1_total.fun(x) / clear_sky_UVA1_total.fun(90)}
...
clear_sky_PAR_total_rat.fun <- function(x) {1}
clear_sky_UVB_total_rat.fun <- 
  function(x) {clear_sky_UVB_total.fun(x) / clear_sky_UVB_total.fun(90) *
      clear_sky_PAR_total.fun(90) / clear_sky_PAR_total.fun(x)}
clear_sky_UVA2_total_rat.fun <- 
  function(x) {clear_sky_UVA2_total.fun(x) / clear_sky_UVA2_total.fun(90) *
      clear_sky_PAR_total.fun(90) / clear_sky_PAR_total.fun(x)}
clear_sky_UVA1_total_rat.fun <- 
  function(x) {clear_sky_UVA1_total.fun(x) / clear_sky_UVA1_total.fun(90) *
      clear_sky_PAR_total.fun(90) / clear_sky_PAR_total.fun(x)}
...
```{r}
#| label: fig-tuv-total-rel
#| echo: false
#| fig-cap: "Simulated PAR and UVB total photon irradiances as a function of solar elevation angle. Computed with the Quick TUV calculator for clear sky conditions.  Points are values obtained by running the TUV model, and the black lines are interpolation splines."
ggplot(subset(TUV.tb, SEA > -3), aes(SEA, total, color = w.band)) +
  stat_function(fun = clear_sky_PAR_total_rel.fun,
                geom = "textline",
                label = "PAR",
                xlim = c(-3, 90), 
                colour = "orange", 
                size = 3,
                linewidth = 1) +
  stat_function(fun = clear_sky_UVB_total_rel.fun, 
                geom = "textline",
                label = "UVB",
                xlim = c(-3, 90), 
                colour = "violet", 
                size = 3,
                linewidth = 1) +
  stat_function(fun = clear_sky_UVA2_total_rel.fun, 
                geom = "textline",
                label = "UVA2",
                xlim = c(-3, 90), 
                colour = "purple", 
                size = 3,
                linewidth = 1) +
  stat_function(fun = clear_sky_UVA1_total_rel.fun, 
                geom = "textline",
                label = "UVA1",
                xlim = c(-3, 90), 
                colour = "blue3", 
  ...
  scale_x_continuous(name = "Solar elevation angle (degrees)",
                     breaks = c(-10, 0, 15, 30, 45, 60, 75, 90)) +
  scale_y_continuous(name = "Photon irradiance (rel. units)") -> qirrad_sun_elevation.fig
qirrad_sun_elevation.fig
```

```{r}
pdf("qirrad_sun_elevation.fig.pdf", width = 6, height = 4)
print(qirrad_sun_elevation.fig)
dev.off()
```
...
```{r}
#| label: fig-tuv-total-rat
#| echo: false
#| fig-cap: "Simulated PAR and UVB total photon irradiances as a function of solar elevation angle. Computed with the Quick TUV calculator for clear sky conditions.  Points are values obtained by running the TUV model, and the black lines are interpolation splines."
ggplot(subset(TUV.tb, SEA > -3), aes(SEA, total, color = w.band)) +
  stat_function(fun = clear_sky_PAR_total_rat.fun, xlim = c(3, 90), colour = "orange", linewidth = 1) +
  stat_function(fun = clear_sky_UVB_total_rat.fun, xlim = c(3, 90), colour = "violet", linewidth = 1) +
  stat_function(fun = clear_sky_UVA2_total_rat.fun, xlim = c(3, 90), colour = "purple", linewidth = 1) +
  stat_function(fun = clear_sky_UVA1_total_rat.fun, xlim = c(3, 90), colour = "blue3", linewidth = 1) +
#  geom_point(na.rm = TRUE) +
  scale_y_log10(name = "Photon irradiance (rel. units)",
                breaks = c(1, 0.5, 0.2, 0.1, 0.05)) +
  scale_x_continuous(name = "Solar elevation angle (degrees)",
                     breaks = c(-10, 0, 15, 30, 45, 60, 75, 90))
```
...
ggplot(TUV.tb, aes(SEA, diff_fraction)) +
  stat_function(fun = clear_sky_PAR_diff_frac.fun, 
                ...
                size = 3,
                linewidth = 1) +
  stat_function(fun = clear_sky_UVB_diff_frac.fun, 
                geom = "textline",
                label = "UVB",
                xlim = c(-3, 90), 
                colour = "violet", 
                size = 3,
                linewidth = 1) +
  stat_function(fun = clear_sky_UVA2_diff_frac.fun, 
                geom = "textline",
                label = "UVA2",
                xlim = c(-3, 90), 
                colour = "purple", 
                size = 3,
                linewidth = 1) +
  stat_function(fun = clear_sky_UVA1_diff_frac.fun, 
                geom = "textline",
                label = "UVA1",
                xlim = c(-3, 90), 
                colour = "blue3",
  ...
  expand_limits(y = 0)  -> diff_fr_sun_elevation.fig
diff_fr_sun_elevation.fig
```

```{r}
pdf("diff_qirrad_sun_elevation.fig.pdf", width = 6, height = 4)
print(diff_fr_sun_elevation.fig)
dev.off()
```
...
minutes.df |>
  ...
  facet_grid(cols = vars(PAR_scattering)) -> par_sun_elevation.fig
par_sun_elevation.fig
```
...
minutes.df |>
  ...
  na.omit() |>
  ggplot(aes(PAR_umol_BF, UVA2_umol)) +
  geom_hdr(probs = kde_probs) +
  stat_panel_counts(label.x = "left", size = 2.7) +
  expand_limits(y =  1e-6, x = 1e-3) +
  scale_y_log10(breaks= c(100, 30, 10, 3,1,0.3,0.1,0.03,0.01, 0.003, 0.001, 0.0003)) +
  scale_x_log10(breaks = c(3000, 1000, 300, 100, 30, 10, 3, 1)) +
  labs(y = expression("UV-A2 photon irradiance  "*(mu*mol~m^{-2}~s^{-1})),
       x = expression("PAR photon irradiance  "*(mu*mol~m^{-2}~s^{-1}))) +
  coord_cartesian(xlim = c(10, 2000), ylim = c(0.3, 30)) +
  facet_grid(cols = vars(PAR_scattering))
```
...
```{r, fig.asp=0.7}
minutes.df |>
  select(PAR_umol_BF, UVA1_umol, PAR_scattering) |>
  na.omit() |>
  ggplot(aes(PAR_umol_BF, UVA1_umol)) +
  geom_hdr(probs = kde_probs) +
  stat_panel_counts(label.x = "left", size = 2.7) +
  expand_limits(y =  1e-6, x = 1e-3) +
  scale_y_log10(breaks= c(300, 100, 30, 10, 3,1,0.3,0.1,0.03,0.01, 0.003, 0.001, 0.0003)) +
  scale_x_log10(breaks = c(3000, 1000, 300, 100, 30, 10, 3, 1)) +
  labs(y = expression("UV-A1 photon irradiance  "*(mu*mol~m^{-2}~s^{-1})),
       x = expression("PAR photon irradiance  "*(mu*mol~m^{-2}~s^{-1}))) +
  coord_cartesian(xlim = c(10, 2000), ylim = c(1, 200)) +
  facet_grid(cols = vars(PAR_scattering))
```
...
```{r}
#| label: fig-diffuse-joint-par-pooled-2
#| echo: false
#| fig-height: 7
#| fig-asp: 1
#| fig-cap: "Probabilty density function summarizing the observed relationship between UV-B photon irradiance and PAR photon irradiance pooled over seven months (April to October), under two different cloud cover conditions, as well as jointly. Data from years 2019 to 2023, 1 April to 30 October."
...
minutes.df |>
  select(PAR_umol_BF, UVB_umol, UVA2_umol, UVA1_umol, PAR_scattering) |>
  na.omit() |>
  ggplot(aes(PAR_umol_BF, UVB_umol)) +
  geom_hdr(aes(PAR_umol_BF, UVB_umol), probs = kde_probs, 
           fill = "violet", colour = "violet", linewidth = 0.1) +
  geom_hdr(aes(PAR_umol_BF, UVA2_umol), probs = kde_probs, 
           fill = "purple", colour = "purple", linewidth = 0.1,
           show.legend = FALSE) +
  geom_hdr(aes(PAR_umol_BF, UVA1_umol), probs = kde_probs, 
           fill = "blue", colour = "blue", linewidth = 0.1,
           show.legend = FALSE) +
  stat_function(fun = function(x){x / 10}, linetype = "dashed") +
  stat_function(fun = function(x){x / 30}, linetype = "dotted") +
  stat_function(fun = function(x){x / 100}, linetype = "dashed") +
  stat_function(fun = function(x){x / 300}, linetype = "dotted") +
  stat_function(fun = function(x){x / 1000}, linetype = "dashed") +
  stat_function(fun = function(x){x / 3000}, linetype = "dotted") +
  annotate(geom = "label", 
           x = c(rep(10, 5), 30),
           y = c(1, 0.1, 0.01, 0.3333, 0.0333, 0.01), 
           label = c("1:10", "1:100", "1:1000", "1:30", "1:300", "1:3000"), 
           hjust = 0.5, size = 2.7, angle = 40) + # 49.5 for asp 1.2
  stat_panel_counts(label.x = "left", size = 2.8) +
  expand_limits(y = 1e-6, x = c(1e-3, 2)) +
  scale_y_log10(breaks= c(100, 30, 10, 3, 1,0.3,0.1,0.03,0.01, 0.003, 0.001, 0.0003),
                labels = scales::label_number()) +
  scale_x_log10(breaks = c(3000, 1000, 300, 100, 30, 10, 3, 1)) +
  labs(y = expression("UV-A1, UV-A2, and UV-B photon irradiances  "*(mu*mol~m^{-2}~s^{-1})),
       x = expression("PAR photon irradiance  "*(mu*mol~m^{-2}~s^{-1})),
       alpha = "Probability") +
  coord_cartesian(xlim = c(8, 2000), ylim = c(0.01, 200)) +
  facet_grid(cols = vars(PAR_scattering)) +
  theme(panel.grid.minor = element_blank(),
        legend.position = "right",
        legend.title.position = "left",
        legend.title = element_text(angle = 90, hjust = 0.5)
) -> uv_par_loglog_ratios.fig
uv_par_loglog_ratios.fig
```

```{r}
pdf("uv_par_loglog_ratios.fig.pdf", width = 9, height = 6.3)
print(uv_par_loglog_ratios.fig)
dev.off()
```


```{r}
#| label: fig-diffuse-joint-par-
#| echo: false
#| fig-width: 7
#| fig-asp: 1
#| fig-cap: "Probabilty density function summarizing the observed relationship between UV-B photon irradiance and PAR photon irradiance pooled over seven months (April to October), under two different cloud cover conditions, as well as jointly. Data from years 2019 to 2023, 1 April to 30 October."
...
minutes.df |>
  ...
  geom_hdr(aes(PAR_umol_BF, UVA2_umol), probs = kde_probs, 
           fill = "purple", colour = "purple", linewidth = 0.1,
           show.legend = FALSE) +
  geom_hdr(aes(PAR_umol_BF, UVA1_umol), 
           probs = kde_probs, 
           fill = "blue", colour = "blue", linewidth = 0.1,
           show.legend = FALSE) +
  # annotate(geom = "label", 
  #          x = rep(10, 3),
  #          y = c(30, 3, 0.3),
  #          label = c("UV-A1", "UV-A2", "UV-B"),
  #          colour = c("blue", "purple", "violet")) +
  annotate(geom = "label", 
           x = 10,
           y = 30,
           angle = 90,
  ...
  theme(panel.grid.minor = element_blank(),
        legend.position = "right",
        legend.title.position = "left",
        legend.title = element_text(angle = 90, hjust = 0.5)) -> uv_par_loglog_labels.fig
uv_par_loglog_labels.fig
```

```{r}
pdf("uv_par_loglog_labels.fig.pdf", width = 9, height = 6.3)
print(uv_par_loglog_labels.fig)
dev.off()
```
```{r}
#| label: fig-diffuse-joint-UVA1-pooled-2
#| echo: false
#| fig-height: 7
#| fig-asp: 1
#| fig-cap: "Probabilty density function summarizing the observed relationship between UV-B photon irradiance and PAR photon irradiance pooled over seven months (April to October), under two different cloud cover conditions, as well as jointly. Data from years 2019 to 2023, 1 April to 30 October."
...
minutes.df |>
  select(UVA1_umol, UVB_umol, UVA2_umol, PAR_scattering) |>
  na.omit() |>
  ggplot(aes(UVA1_umol, UVB_umol)) +
  geom_hdr(aes(UVA1_umol, UVB_umol), probs = kde_probs, 
           fill = "violet", colour = "violet", linewidth = 0.1) +
  geom_hdr(aes(UVA1_umol, UVA2_umol), probs = kde_probs, 
           fill = "purple", colour = "purple", linewidth = 0.1,
           show.legend = FALSE) +
  stat_function(fun = function(x){x / 3}, linetype = "dotted") +
  stat_function(fun = function(x){x / 10}, linetype = "dashed") +
  stat_function(fun = function(x){x / 30}, linetype = "dotted") +
  stat_function(fun = function(x){x / 100}, linetype = "dashed") +
  stat_function(fun = function(x){x / 300}, linetype = "dotted") +
  annotate(geom = "label", 
           x = c(rep(3, 5)),
           y = c(1, 0.333, 0.0333, 0.1, 0.01), 
           label = c("1:3", "1:10", "1:100", "1:30", "1:300"), 
           hjust = 0.5, size = 2.7, angle = 45) + # 49.5 for asp 1.2
  stat_panel_counts(label.x = "left", size = 2.8) +
  expand_limits(y = 1e-6, x = c(1, 200)) +
  scale_y_log10(breaks= c(100, 30, 10, 3, 1,0.3,0.1,0.03,0.01, 0.003, 0.001, 0.0003),
                labels = scales::label_number()) +
  scale_x_log10(breaks = c(3000, 1000, 300, 100, 30, 10, 3, 1)) +
  labs(y = expression("UV-A2, and UV-B photon irradiances  "*(mu*mol~m^{-2}~s^{-1})),
       x = expression("UV-A1 photon irradiance  "*(mu*mol~m^{-2}~s^{-1})),
       alpha = "Probability") +
  coord_cartesian(xlim = c(1, 150), ylim = c(0.01, 50)) +
  facet_grid(cols = vars(PAR_scattering)) +
  theme(panel.grid.minor = element_blank(),
        legend.position = "right",
        legend.title.position = "left",
        legend.title = element_text(angle = 90, hjust = 0.5)
) -> uv_uva1_loglog_ratios.fig
uv_uva1_loglog_ratios.fig
```


```{r}
pdf("uv_uva1_loglog_labels.fig.pdf", width = 9, height = 6.3)
print(uv_uva1_loglog_ratios.fig)
dev.off()
```
...
ggplot(daily_UVB_hours_long.tb, aes(month, Duration)) +
  ...
  facet_wrap(facets = vars(Threshold.units), labeller = label_parsed) -> daily_UVB_hours.fig
daily_UVB_hours.fig
```

```{r}
pdf("daily_UVB_hours.fig.pdf", width = 8, height = 5)
print(daily_UVB_hours.fig)
dev.off()
```
